...
def extract_3yr_upfront_price(price_data):
    ...
    for term in reserved_terms.values():
        ...
        if (term_attrs.get('LeaseContractLength') == '3yr' and 
            term_attrs.get('PurchaseOption') == 'All Upfront' and
            term_attrs.get('OfferingClass') == 'standard'):
...
def fetch_pricing():
    ...
    output_file = os.path.join(project_root, 'service_capacity_modeling', 'hardware', 'profiles', 'pricing', 'aws', '3yr-reserved_ec2.json')
...
def main():
    parser = argparse.ArgumentParser(description="Fetch EC2 Reserved Instance pricing data.")
