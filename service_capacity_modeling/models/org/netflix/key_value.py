...
class NflxKeyValueCapacityModel(CapacityModel, CostAwareModel):
    ...
    def cluster_costs(
        ...
        return cluster_infra_cost(
            ...
            cluster_types=NflxKeyValueCapacityModel.cluster_types,
        )
    ...
    def service_costs(
        ...
        """Calculate service costs for dgwkv layer only.

        The dgwkv layer is a stateless Java app - it doesn't replicate data itself,
        so it has no additional service costs (matching nflx-java-app behavior).
        The planner routes Cassandra/EVCache service costs automatically via
        _sub_models() DAG traversal.
        """
