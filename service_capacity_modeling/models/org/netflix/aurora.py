...
def _compute_aurora_region(  # pylint: disable=too-many-positional-arguments
    ...
    # to ensure consistent cost calculation between recommendations and baseline extraction
...
class NflxAuroraArguments(BaseModel):
    ...
    aurora_replicas: Optional[int] = Field(
        ...
        description=(
            ...
            "or 1 for tier 3+ (writer only). "
            "For baseline extraction, pass the actual replica count from your deployment."
        ),
    )
...
class NflxAuroraCapacityModel(CapacityModel):
    ...
    def service_costs(
        ...
        IO prices are obtained from:
        1. extra_model_arguments (aurora.read_io_price, aurora.write_io_price)
           - Used when called from capacity_plan() which has access to the drive
        2. context.drives (fallback)
           - Used when called from baseline extraction where drives are in context
        """
        db_type = extra_model_arguments.get("aurora.engine", "postgres")

        # Get IO prices - either passed directly or look up from context.drives
        ...
        if read_io_price is None or write_io_price is None:
            # Look up from context.drives (populated by capacity_planner)
            aurora_drive = context.drives.get("aurora")
            if aurora_drive is not None:
                read_io_price = aurora_drive.annual_cost_per_read_io[0][1]
                write_io_price = aurora_drive.annual_cost_per_write_io[0][1]

        # If we still don't have IO prices, return empty (no IO costs)
        if read_io_price is None or write_io_price is None:
            return []
