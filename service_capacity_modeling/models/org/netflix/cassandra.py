...
def _estimate_cassandra_cluster_zonal(  # pylint: disable=too-many-positional-arguments
    ...
    # Update the requirement to reflect what was actually used for sizing.
    # With soft memory the cluster was sized without a memory constraint;
    # set mem_gib to the actual page cache the cluster provides.
    if soft_memory:
        actual_cache = available_cache * cluster.count
        requirement = requirement.model_copy(
            update={"mem_gib": certain_float(actual_cache)}
        )
