...
class CapacityPlanner:
    def __init__(
        ...
        # Maps cluster_type -> CostAwareModel for conflict detection
        self._cluster_type_to_model: Dict[str, CostAwareModel] = {}
    ...
    def register_model(self, name: str, capacity_model: CapacityModel) -> None:
        ...
        # Validate cluster_type uniqueness (catches typos, duplicate declarations)
        if isinstance(capacity_model, CostAwareModel):
            for ct in getattr(capacity_model, "cluster_types", ()):
                if ct in self._cluster_type_to_model:
                    existing = self._cluster_type_to_model[ct]
                    raise ValueError(
                        f"Cluster type '{ct}' conflict: "
                        f"'{name}' vs '{existing.service_name}'"
                    )
                self._cluster_type_to_model[ct] = capacity_model
    ...
    def _calculate_costs_via_composition(
        self,
        *,
        model_name: str,
        ...
        """Route both cluster_costs and service_costs via composition tree.

        Uses _sub_models() to walk the DAG - same as plan_certain().
        Each model's cost methods filter by their own cluster_types.

        Args:
            model_name: Top-level model name to start DAG traversal
            context: Region context with services for network costs
            desires: Capacity desires (for service_costs transformations)
            zonal_clusters: All zonal clusters (models filter by cluster_type)
            regional_clusters: All regional clusters (models filter by cluster_type)
            requirement: Aggregated requirement for service costs (e.g., backup)
            extra_model_arguments: Model-specific arguments

        Returns:
            Tuple of (costs dict, services list) aggregated from all models in DAG
        """
        costs: Dict[str, float] = {}
        services: List[ServiceCapacity] = []

        # Walk the composition tree (same DAG as plan generation)
    ...
    def extract_baseline_plan(  # pylint: disable=too-many-positional-arguments
        ...
        # Get cluster_type from model (matches what capacity_plan sets)
        cluster_type = getattr(model, "cluster_types", (model.service_name,))[0]
        ...
        costs, services = self._calculate_costs_via_composition(
            model_name=model_name,
            context=context,
            desires=desires,
