...
class CapacityPlanner:
    ...
    def register_model(self, name: str, capacity_model: CapacityModel) -> None:
        # Duplicate cluster_type would cause double-counting in cost calculation
        if isinstance(capacity_model, CostAwareModel):
            new_ct = getattr(capacity_model, "cluster_type", "")
            if new_ct:
                for existing_name, existing_model in self._models.items():
                    if isinstance(existing_model, CostAwareModel):
                        existing_ct = getattr(existing_model, "cluster_type", "")
                        if existing_ct == new_ct:
                            raise ValueError(
                                f"Duplicate cluster_type '{new_ct}': '{name}' "
    ...
    def extract_baseline_plan(  # pylint: disable=too-many-positional-arguments
        ...
        cluster_type = getattr(model, "cluster_type", "") or model.service_name
