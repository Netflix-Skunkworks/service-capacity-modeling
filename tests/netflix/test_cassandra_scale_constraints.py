...
class I4i4xlarge:
    ...
    def __init__(self, cluster_size: int = 4):
        ...
        self.vcpu_per_instance = 16
        self.ram_gib_per_instance = 128
        self.disk_gib_per_instance = 3750  # 3.75 TB
    ...
    def memory(self) -> ResourceTargets:
        ...
        ideal_ram = int(self.total_ram_gib * 0.12)  # ~12% of total RAM
    ...
    def disk(self) -> ResourceTargets:
        ...
        ideal_storage = int(self.total_disk_gib / 4)  # 4x buffer
    ...
    def network(self) -> ResourceTargets:
        ...
        # Apply default compute buffer ratio 1.5x
        ideal_network = int(inst.net_mbps * 1.5) * self.cluster_size
    ...
    def get(
        ...
        return CurrentZoneClusterCapacity(
            ...
            cpu_utilization=certain_float(cpu if cpu is not None else self.cpu.ideal),
            memory_utilization_gib=certain_float(
                memory_gib if memory_gib is not None else self.memory.ideal
            ),
            disk_utilization_gib=certain_float(
                disk_gib if disk_gib is not None else self.disk.ideal
            ),
            network_utilization_mbps=certain_float(
                network if network is not None else self.network.ideal
            ),
        )

    def get_desires(
        self, cluster, state_size_gib: int, buffer: Buffers
    ) -> CapacityDesires:
...
class TestStorageScalingConstraints:
    ...
    def test_storage_running_hot_scale_up(self):
        """
        ...
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.hot, self.STORAGE_SCALE_UP
        )
    ...
    def test_storage_over_provisioned_scale_down(self):
        """
        Test: Storage is over-provisioned (below cold threshold) with scale_down intent

        EXPECTATION: Storage should scale down to meet 4x buffer requirement
        WHY: Current storage is excessive, so scale_down should reduce it
             to the minimum needed
        """
        cluster_capacity = self.DISK_COLD_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.cold, self.STORAGE_SCALE_DOWN
        )
        ...
        # Storage should meet 4x buffer requirement
        expected_storage = CLUSTER.disk.cold * 4  # 4x buffer on cold state size
        assert result_storage >= expected_storage, (
    ...
    def test_storage_cold_scale_up(self):
        ...
        cluster_capacity = self.DISK_COLD_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.cold, self.STORAGE_SCALE_UP
        )
    ...
    def test_storage_hot_scale_down(self):
        """
        Test: Storage is hot (above ideal) with scale_down intent

        EXPECTATION: scale_down should prevent scaling up above current levels
        WHY: scale_down intent means "only scale down, never scale up" -
            this tests the constraint
        """
        cluster_capacity = self.DISK_HOT_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.hot, self.STORAGE_SCALE_DOWN
        )
        ...
        # scale_down should prevent scaling up above current levels
        # Even if demand is high, we should not exceed current storage capacity
        assert result_storage <= CLUSTER.total_disk_gib, (
            f"scale_down should prevent scaling up above current "
            f"{CLUSTER.total_disk_gib} GiB, got {result_storage} GiB"
        )
...
class TestCPUScalingConstraints:
    ...
    def test_cpu_running_cool_scale_up(self):
        ...
        WHY: Current CPU is under-utilized, so scale_up should expand it
        """
        cluster_capacity = self.CPU_COLD_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.ideal, self.CPU_SCALE_UP
        )
    ...
    def test_cpu_running_cool_scale_down(self):
        ...
        cluster_capacity = self.CPU_COLD_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.ideal, self.CPU_SCALE_DOWN
        )
        ...
        expected_cpu_cores = (
            CLUSTER.cpu.cold * 1.5
        )  # 1.5x buffer on cold CPU utilization
        assert result_cores >= expected_cpu_cores, (
            f"CPU should meet 1.5x buffer requirement of {expected_cpu_cores} cores, "
            f"got {result_cores} cores"
        )

        # CPU should scale down from current over-provisioned level
        assert result_cores <= CLUSTER.total_vcpu, (
            f"CPU should scale down from {CLUSTER.total_vcpu} cores, "
            f"got {result_cores} cores"
        )
    ...
    def test_cpu_running_hot_scale_up(self):
        ...
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.hot, self.CPU_SCALE_UP
        )
    ...
    def test_cpu_running_hot_scale_down(self):
        ...
        EXPECTATION: CPU should scale down to meet 1.5x buffer requirement
        WHY: Even though CPU is hot, scale_down intent allows reduction to meet demand
        """
        cluster_capacity = self.CPU_HOT_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.ideal, self.CPU_SCALE_DOWN
        ...
        # CPU should scale down to meet 1.5x buffer requirement
        expected_cpu_cores = CLUSTER.cpu.hot * 1.5  # 1.5x buffer on hot CPU utilization
        assert result_cores >= expected_cpu_cores, (
            f"CPU should meet 1.5x buffer requirement of {expected_cpu_cores} cores, "
            f"got {result_cores} cores"
        )
    ...
    def test_cpu_over_provisioned_scale_up_constraint(self):
        """
        Test: CPU is over-provisioned (running cool) with scale_up intent
        ...
        cluster_capacity = self.CPU_COLD_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.cold, self.CPU_SCALE_UP
        )
        ...
        # scale_up should prevent scaling down below current levels
        # Even if demand is low, we should maintain at least current capacity
        assert result_cores >= CLUSTER.total_vcpu, (
            f"scale_up should prevent scaling down below current "
            f"{CLUSTER.total_vcpu} cores, got {result_cores} cores"
        )

    def test_cpu_under_provisioned_scale_down_constraint(self):
        """
        Test: CPU is under-provisioned (running hot) with scale_down intent

        EXPECTATION: scale_down should prevent scaling up above current levels
        WHY: scale_down intent means "only scale down, never scale up" - this
        tests the constraint
        """
        cluster_capacity = self.CPU_HOT_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.hot, self.CPU_SCALE_DOWN
        )
        ...
        # scale_down should prevent scaling up above current levels
        # Even if demand is high, we should not exceed current capacity
        assert result_cores <= CLUSTER.total_vcpu, (
            f"scale_down should prevent scaling up above current "
            f"{CLUSTER.total_vcpu} cores, got {result_cores} cores"
        )
...
class TestStorageAndCPUIntegration:
    ...
    def test_cpu_scale_up_disk_scale_up(self):
        ...
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.ideal, self.BOTH_SCALE_UP
        )
        ...
        # Storage should scale up to meet 4x buffer requirement
        expected_storage = CLUSTER.disk.hot * 4  # 4x buffer on hot disk utilization
        assert result_storage >= expected_storage, (
            f"Storage should scale up to at least {expected_storage} GiB, "
            f"got {result_storage} GiB"
        )
    ...
    def test_cpu_scale_down_disk_scale_down(self):
        ...
        EXPECTATION: Both CPU and storage should scale down to meet their requirements
        WHY: This tests the realistic scenario where both components can be reduced
        """
        cluster_capacity = self.BOTH_CONSTRAINED_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.cold, self.BOTH_SCALE_DOWN
        )
        ...
        # Storage should meet 4x buffer requirement
        expected_storage = CLUSTER.disk.cold * 4  # 4x buffer on cold disk utilization
        assert result_storage >= expected_storage, (
            f"Storage should meet 4x buffer requirement of {expected_storage} GiB, "
            f"got {result_storage} GiB"
        )

        # Both should scale down from current over-provisioned levels
        assert result_cores <= CLUSTER.total_vcpu, (
            f"CPU should scale down from {CLUSTER.total_vcpu} cores, "
            f"got {result_cores} cores"
        )
        assert result_storage <= CLUSTER.total_disk_gib, (
            f"Storage should scale down from {CLUSTER.total_disk_gib} GiB, "
            f"got {result_storage} GiB"
        )
    ...
    def test_cpu_scale_up_disk_scale_down(self):
        ...
        cluster_capacity = self.BOTH_CONSTRAINED_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.ideal, self.CPU_UP_DISK_DOWN
        )
        cap_plan = planner.plan_certain(
            model_name="org.netflix.cassandra",
            region="us-east-1",
        ...
        assert result_storage <= CLUSTER.total_disk_gib, (
            f"Storage should not exceed current "
            f"{CLUSTER.total_disk_gib} GiB, got {result_storage} GiB"
        )
    ...
    def test_cpu_scale_down_disk_scale_up(self):
        ...
        cluster_capacity = self.BOTH_CONSTRAINED_CAPACITY
        desires = CLUSTER.get_desires(
            cluster_capacity, CLUSTER.disk.ideal, self.CPU_DOWN_DISK_UP
        )
        cap_plan = planner.plan_certain(
            model_name="org.netflix.cassandra",
            region="us-east-1",
        ...
        result_cores = result.count * result.instance.cpu
        result_storage = result.count * result.instance.drive.size_gib
        # CPU should not exceed current capacity
        assert result_cores <= CLUSTER.total_vcpu, (
            f"CPU should not exceed current "
            f"{CLUSTER.total_vcpu} cores, got {result_cores} cores"
        )
        # Storage should scale up to meet 4x buffer requirement
        expected_storage = CLUSTER.disk.hot * 4
        assert result_storage >= expected_storage, (
            f"Storage should scale up to at least {expected_storage} GiB, "
            f"got {result_storage} GiB"
        )
