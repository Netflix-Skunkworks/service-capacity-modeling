...
Without any regret penalties, the planner sorts by (rank, cost). Within the
m6id family, AWS 3yr pricing rounding makes larger instances appear marginally
cheaper, so they win. This is the problem that large_instance_regret (PR #230)
exists to correct.
...
class TestUnpenalizedBaseline:
    """Without regret penalties, AWS pricing rounding favors larger instances."""

    def test_unpenalized_prefers_large_instance_within_family(self):
        ...
        assert normalized_aws_size(result.instance.name) > 8, (
            f"Expected a large m6id instance without regret, got {result.instance.name}"
        )
