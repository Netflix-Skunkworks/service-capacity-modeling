...
# Workload large enough that 16xlarge ranks first without regret (due to
# AWS pricing rounding favoring larger instances).
...
class TestDefaultRegretFlipsOrdering:
    ...
    def test_8xlarge_ranks_above_16xlarge_by_default(self):
        """With default regret, m6id.8xlarge should beat m6id.16xlarge."""
...
class TestRegretCanBeDisabled:
    """Setting large_instance_regret=0 should restore raw cost ordering."""

    def test_no_regret_allows_larger_instances_to_win(self):
        """With regret=0, m6id.32xlarge ranks above m6id.8xlarge (rounding wins)."""
        ...
        # Without regret: larger instances rank better due to rounding
        assert idx_32xl < idx_16xl < idx_8xl, (
            f"Expected 32xl({idx_32xl}) < 16xl({idx_16xl}) < 8xl({idx_8xl}) "
            f"with regret=0, but ordering was: {names}"
        )


class TestRegretDoesNotAffectSmallInstances:
    """Instances at 8xlarge and below should not be penalized."""

    def test_costs_unchanged_for_small_instances(self):
        """Costs for <=8xlarge should be identical with and without regret."""
        ...
        def costs_at_or_below_8xl(order):
            return {
                name: cost for name, _, cost in order if normalized_aws_size(name) <= 8
            }
        ...
        for name in costs_without:
            if name in costs_with:
                assert costs_without[name] == costs_with[name], (
                    f"{name} cost changed with regret: "
