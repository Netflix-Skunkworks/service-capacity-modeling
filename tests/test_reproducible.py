...
from service_capacity_modeling.explainability import MergedRegretCandidate
from service_capacity_modeling.explainability import _candidate_shape_id
...
from service_capacity_modeling.explainability import RegretCandidate
from service_capacity_modeling.explainability import (
    summaries_for_least_regret,
)
from service_capacity_modeling.explainability import summarize_regret_candidates
...
def test_composed_explained_samples_exist_in_all_models():
    ...
    for plan, summary in zip(
        ...
        assert _candidate_shape_id(summary.plan) == _candidate_shape_id(plan)

    expected_model_names = set(explained.plan.explanation.regret_clusters_by_model)
    assert expected_model_names
    assert set(explained.excuses_by_model).issubset(expected_model_names)
    assert explained.excuses_by_model == explained.plan.explanation.excuses_by_model
    ...
    selected_shape_ids = {
        _candidate_shape_id(summary.plan)
        for summary in explained.least_regret_summaries
    }
    alternative_shape_ids = {
        _candidate_shape_id(summary.plan)
    ...
    assert selected_shape_ids.isdisjoint(alternative_shape_ids)
    for summary in explained.considered_alternatives:
        assert set(summary.mean_regret_components_by_model) == expected_model_names

    excuse_source_models = {excuse.source_model for excuse in explained.excuse_summary}
    assert excuse_source_models
    assert excuse_source_models.issubset(expected_model_names)
...
def test_composed_certain_explained_keeps_excuses_by_model():
    ...
    assert explained.excuses_by_model
    assert set(explained.excuses_by_model).issubset(
        {"org.netflix.key-value", "org.netflix.cassandra"}
    )
    for model, excuses in explained.excuses_by_model.items():
        assert model
        assert excuses
...
def test_composed_regret_merge_rejects_empty_model_counts():
    plan = planner.plan(
        model_name="org.netflix.cassandra",
        region="us-east-1",
        desires=uncertain_mid,
...
def test_composed_regret_merge_truncates_unequal_nonzero_model_counts():
    plan = planner.plan(
        model_name="org.netflix.cassandra",
        region="us-east-1",
        desires=uncertain_mid,
    ...
    first = RegretCandidate(
        sample=SampleRef(sample_id="a-1", sample_label="first"),
        plan=plan,
        desires=uncertain_mid,
        total_regret=1.0,
    )
    second = RegretCandidate(
        sample=SampleRef(sample_id="b-1", sample_label="second"),
        plan=plan,
        desires=uncertain_mid,
        total_regret=2.0,
    )
    extra = RegretCandidate(
        sample=SampleRef(sample_id="b-2", sample_label="extra"),
        plan=plan,
        desires=uncertain_mid,
        total_regret=3.0,
    )
    ...
    merged = merge_regret_candidates_positional(
        regret_details_by_model={"a": [first], "b": [second, extra]},
        zonal_requirements={},
        regional_requirements={},
    )

    assert len(merged) == 1
    assert [sample.sample_id for sample in merged[0].samples] == ["a-1", "b-1"]
...
def _ddb_plan_with_capacity(value: str) -> CapacityPlan:
    return CapacityPlan(
...
def test_candidate_shape_id_keeps_service_parameters():
    assert _candidate_shape_id(_ddb_plan_with_capacity("small")) != _candidate_shape_id(
        _ddb_plan_with_capacity("large")
    )
...
def test_regret_summary_accumulates_samples_and_components():
    ...
    assert _candidate_shape_id(plan) != _candidate_shape_id(plan_with_different_params)
    assert len(summaries) == 2

    summary = summaries[_candidate_shape_id(plan)]
    assert summary.sample_count == 2
    assert summary.mean_total_regret == 7.0
    assert "selected_total_regret" not in summary.model_dump()
...
def test_considered_alternatives_are_sorted_by_mean_regret():
    high_mean = _ddb_plan_with_capacity("high-mean")
    low_mean = _ddb_plan_with_capacity("low-mean")
    summaries = summarize_regret_candidates(
        [
            MergedRegretCandidate(
                samples=[SampleRef(sample_id="s-1", sample_label="first")],
    ...
    alternatives = considered_alternative_summaries(
        least_regret=[],
        regret_summary_map=summaries,
        max_results=2,
    )

    assert [summary.plan for summary in alternatives] == [low_mean, high_mean]
    assert [summary.mean_total_regret for summary in alternatives] == [2.0, 51.0]
